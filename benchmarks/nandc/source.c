...
const char* bll(bool x) {
    // Refinator: Changed code since Refinator's LLVM pass doesn't handle Select instructions.
    if (x)
        return "true";
    return "false";
    //return x ? "true" : "false";
...
int main(void) {
    // Refinator: Removed asserts since LLVM pass doesn't handle Unreachable instructions.
    /*assert*/(check_add());
    /*assert*/(check_sub());
    return 0;
}
