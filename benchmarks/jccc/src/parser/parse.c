...
int parse(const char *filename) {
    ...
    int i = 0;
    int buffer_size = 16;
    Token *tokens = malloc(buffer_size * sizeof(Token)); // &inator: changed to malloc to avoid constraint generation error
    ...
    do {
        ...
        if (buffer_size <= i) {
            buffer_size *= 2;
            tokens = malloc(buffer_size * sizeof(Token)); // &inator: changed to malloc to avoid constraint generation error
        }
