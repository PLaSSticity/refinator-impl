...
// &inator: Monomorphized `malloc` and `free` ----------------------------------
rbtree *REFINATOR_MALLOC_rbtree();
node_t *REFINATOR_MALLOC_node_t();
int REFINATOR_FREE_rbtree(rbtree *t);
int REFINATOR_FREE_node_t(node_t *n);
// -----------------------------------------------------------------------------
...
rbtree *new_rbtree(void)
{
    rbtree *p = REFINATOR_MALLOC_rbtree();
    node_t *nilNode = REFINATOR_MALLOC_node_t();
...
void REFINATOR_FREE_node(rbtree *t, node_t *x)
{
    // 후위 순회 방식으로 RB Tree 내의 노드 메모리 반환
    if (x->left != t->nil)
        REFINATOR_FREE_node(t, x->left);
    if (x->right != t->nil)
        REFINATOR_FREE_node(t, x->right);
    REFINATOR_FREE_node_t(x);
    x = NULL;
}
...
void delete_rbtree(rbtree *t)
{
    ...
    if (t->root != t->nil)
        REFINATOR_FREE_node(t, t->root);
    REFINATOR_FREE_node_t(t->nil);
    REFINATOR_FREE_rbtree(t);
}
...
node_t *rbtree_insert(rbtree *t, const key_t key)
{
    ...
    node_t *y = t->nil;
    node_t *x = t->root;
    node_t *z = REFINATOR_MALLOC_node_t();
