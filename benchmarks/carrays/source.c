...
void gca_merge(void *_dst, size_t ndst, size_t nsrc, size_t es,
               int compar_id /*int (*compar)(const void *_a, const void *_b, void *_arg)*/,
               void *arg)
{
  char *dst = (char*)_dst, *src = dst+es*ndst, *end = dst+es*(ndst+nsrc);
  if(!nsrc || !ndst) {}
  else if(call_compar(compar_id, src-es, src, arg) <= 0) {}
  else if(call_compar(compar_id, dst, end-es, arg) >= 0) {
    gca_cycle_left(dst, ndst, es, ndst);
  }
  else if(ndst+nsrc < 6) {
    // insertion sort merge of dst and src
    gca_imerge(dst, ndst, nsrc, es, compar_id, arg);
  }
  else {
    gca_qsort(dst, ndst+nsrc, es, compar_id, arg);
  }
}
...
void* gca_bsearch(void *_ptr, size_t n, size_t es,
                  int searchf_id /*int (*searchf)(const void *_val, void *_arg)*/,
                  void *arg)
{
  if(n == 0) return NULL;
  ...
  while(1)
  {
    ...
    mid = l + (r - l)/2;
    mptr = ptr+es*mid;
    cmp = call_searchf(searchf_id, mptr, arg);
                  ...
                  void *arg)
{
  ...
  for(; b < end; b++)
    if(call_searchf(searchf_id, b,arg) == 0)
      return b;
  return NULL;
}
                 ...
                 void *arg)
{
  ...
  while(pl < pr) {
    for(; pl < pr; pr -= es) {
      if(call_compar(compar_id, pr, pivot, arg) < 0) {
        memcpy(pl, pr, es);
    ...
    for(; pl < pr; pl += es) {
      if(call_compar(compar_id, pl, pivot, arg) > 0) {
        memcpy(pr, pl, es);
  ...
  // now pl == pr
  memcpy(pl, pivot, es);
  return 0 /*(pl-b)es*/; // &inator: eliminated ptrtoint instruction
}
               ...
               void *arg)
{
  ...
  if(nel < 6) {
    ...
    gca_isortr(base, 0, nel, es, compar_id, arg);
  }
  else
  {
    /* Use median of first, middle and last items as pivot */
    char *pivot = gca_median3(b, b+es*(nel/2), b+es*(nel-1), compar_id, arg);

    // swap pivot into first element and partition
    gca_swapm(b, pivot, es);
    size_t pidx = gca_qpart(b, nel, es, compar_id, arg);

    gca_qsort(b, pidx, es, compar_id, arg);
    gca_qsort(b+es*(pidx+1), nel-(pidx+1), es, compar_id, arg);
  }
}
                  ...
                  void *arg)
{
  ...
  {
    /* Use median of first, middle and last items as pivot */
    char *pivot = gca_median3(b+es*l, b+es*(l+(r-l+1)/2), b+es*r, compar_id, arg);

    // swap pivot into first element and partition
    gca_swapm(b+es*l, pivot, es);
    pidx = l + gca_qpart(b+es*l, r-l+1, es, compar_id, arg);
                       ...
                       void *arg)
{
  ...
  for(chi = nel-1; chi > 0; chi = pi) {
    ...
    if(call_compar(compar_id, b+es*pi, tmp, arg) >= 0) break;
                      ...
                      void *arg)
{
  ...
  memcpy(tmp, last, es);
  for(p = b, ch = b+es; ch < last; p = ch, ch += es /* &inator: eliminated ptrtoint: ch = b + 2*(ch-b) + es*/) {
    ch = (ch+es < last && call_compar(compar_id, ch,ch+es,arg) < 0 ? ch+es : ch); // biggest child
    if(call_compar(compar_id,tmp, ch, arg) >= 0) break;
    memcpy(p, ch, es);
  }
  memcpy(p, tmp, es);
}
                   ...
                   void *arg)
{
  ...
  for(n = 2; n <= nel; n++)
    gca_heap_pushup(heap, n, es, compar_id, arg);
}
                   ...
                   void *arg)
{
  ...
  {
    gca_swapm(b, end, es);
    gca_heap_pushdwn(heap, n, es, compar_id, arg);
  }
  gca_swapm(b, b+es, es);
}
...
void* gca_median3(void *p0, void *p1, void *p2,
                  ...
                  void *arg)
{
  if(call_compar(compar_id, p0, p1, arg) > 0) SWAP(p0, p1);
  if(call_compar(compar_id, p1, p2, arg) > 0) {
    SWAP(p1, p2);
    if(call_compar(compar_id, p0, p1, arg) > 0) SWAP(p0, p1);
  }
  return p1;
                  ...
                  void *arg)
{
  ...
  if(call_compar(compar_id, p0, p1, arg) > 0) SWAP(p0, p1);
  // make p2<p3
  if(call_compar(compar_id, p2, p3, arg) > 0) SWAP(p2, p3);

  // get rid of min(min(p0,p1),min(p2,p3)),
  // overwrite with p4
  // make p0<p1, p2<p3 again
  if(call_compar(compar_id, p0, p2, arg) < 0) {
    p0 = p4;
    if(call_compar(compar_id, p0, p1, arg) > 0) SWAP(p0, p1);
  } else {
    p2 = p4;
  ...
  if(call_compar(compar_id, p0, p2, arg) < 0) {
    return (call_compar(compar_id, p1, p2, arg) < 0 ? p1 : p2);
  } else {
    return (call_compar(compar_id, p3, p0, arg) < 0 ? p3 : p0);
  }
}
