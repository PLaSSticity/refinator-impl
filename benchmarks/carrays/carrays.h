...
static int call_compar(int compar_id, const void *a, const void *b, void *arg)
{
  return 0; // TODO: implement function lookup and call
}

static int call_searchf(int compar_id, const void *_val, const void *_arg) {
  return 0; // TODO: implement function lookup and call
}
...
static inline void gca_isortr(void *_ptr, size_t n, size_t m, size_t el,
                              int compar_id /*int (*compar)(const void *_a, const void *_b,
                                            void *_arg)*/,
                              void *arg)
{
  char *b = (char*)_ptr, *pi, *pj, *end = b+el*(n+m);
  for(pi = b+el*n; pi < end; pi += el)
    for(pj = pi; pj > b && call_compar(compar_id,pj-el,pj,arg) > 0; pj -= el)
                              ...
                              void *arg)
{
  ...
  for(pi = b; pi > start; pi -= el)
    for(pj = pi; pj < end && call_compar(compar_id,pj-el, pj, arg) > 0; pj += el)
      gca_swapm(pj-el, pj, el);
}
                                 ...
                                 void *arg)
{
  ...
  for(ptr = b; ptr+es < end; ptr += es)
    if(call_compar(compar_id, ptr, ptr+es, arg) > 0)
      return false;
  return true;
}
                            ...
                            void *arg)
{
  ...
  for(ptr = b+es; ptr < end; ptr += es)
    if(call_compar(compar_id, max, ptr, arg) < 0)
      max = ptr;
  return max;
}
                            ...
                            void *arg)
{
  ...
  for(ptr = b+es; ptr < end; ptr += es)
    if(call_compar(compar_id, min, ptr, arg) > 0)
      min = ptr;
  return min;
}
