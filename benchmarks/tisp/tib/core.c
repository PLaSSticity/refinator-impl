...
prim_eval(Tsp st, Rec env, Val args)
{
	...
	v = tisp_eval(st, st->env, car(args));
	if (v) {
    	return v;
	} else {
	    return st->none;
...
prim_procprops(Tsp st, Rec env, Val args)
{
	...
	case TSP_MACRO:
		...
		rec_add(ret, "name", mk_sym(st, name));
...
tib_env_core(Tsp st)
{
	...
	tsp_env_form(cond);
	tisp_env_add(st, "do", mk_prim(TSP_FORM, global_prim_val /*tisp_eval_body*/, "do"));
	...
	tisp_env_add(st, "Rec", mk_prim(TSP_FORM, global_prim_val /*mk_rec*/, "Rec"));
