...
fn ulidgen_r(ulid: &[Cell<u8>]) {
    ...
    let mut i: i32 = 9;
    while i >= 0 {
        if ulid[i as usize].get() != b32alphabet.as_bytes()[(t % 32) as usize] {
            ulid[i as usize].set(b32alphabet.as_bytes()[(t & 32) as usize]);
            same = false;
        }
        i -= 1;
        t /= 32
    ...
    if same {
        ...
        if let Some(idx) = b32alphabet.as_bytes().iter().position(|c| *c == buf[i as usize].get()) {
            let s = b32alphabet.as_bytes().split_at(idx).1;
    ...
    for i in 0..16 {
        buf[i].set(b32alphabet.as_bytes()[(rnd[i] % 32) as usize]);
    }
}
